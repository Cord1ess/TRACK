...
Writes captures/<name>/:
    ...
    blocks/           16x16-tile mosaics (compact archive form)
...
Safety net, in order:
    ...
    post        coverage vs threshold, traffic present, palette still matches,
                georef round-trip, every file re-read + re-hashed, manifest complete
...
CRITICAL = ("coverage", "traffic_content", "georef", "files", "block_tiles_consistent")
...
def run(cfg: dict, name: str, out_root: Path, log: Log) -> tuple[Path | None, dict]:
    ...
    blocks = tiles_mod.build_blocks(tiles, cfg["zoom"], set(expected), out_dir, cfg, cfg["tile_px"])
    ...
    ck["georef"] = checks.georef_check(blocks, cfg["zoom"], cfg["bbox"])
    ck["block_tiles_consistent"] = {"ok": sum(b["tiles_present"] for b in blocks) == cov["received"],
                                    "detail": f"blocks hold {sum(b['tiles_present'] for b in blocks)}, received {cov['received']}"}
    ...
    # ---- preview + manifest
    r = manifest["tile_range"]
    W, H = (r["x1"] - r["x0"] + 1) * cfg["tile_px"], (r["y1"] - r["y0"] + 1) * cfg["tile_px"]
    Image.MAX_IMAGE_PIXELS = None
    canvas = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    for (x, y), data in tiles.items():
        canvas.alpha_composite(analyze.load_image(data).convert("RGBA"),
                               ((x - r["x0"]) * cfg["tile_px"], (y - r["y0"]) * cfg["tile_px"]))
    white = Image.new("RGBA", canvas.size, (255, 255, 255, 255))
    white.alpha_composite(canvas)
    scale = max(1, max(canvas.size) // 2600 + 1)
    white.convert("RGB").resize((W // scale, H // scale)).save(out_dir / "preview_on_white.png")
