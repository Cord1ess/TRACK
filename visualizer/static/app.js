...
const newestCapture = (list) => list.reduce((a, b) =>
  (Date.parse(b.captured_utc) || 0) > (Date.parse(a.captured_utc) || 0) ? b : a);

function fillCaptureSelect() {
  const sel = $("capture");
...
/* A capture that arrives while the page is open: list it, mark it on the
   timeline, and move to it unless an older one was chosen on purpose. */
...
/* How the collection is going, under the capture controls. */
const ageText = (ms) => {
